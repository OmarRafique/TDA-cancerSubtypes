...
pathway_analysis = function(cluster=RC[[1]]$cluster,red_data){
  ...
  for(i in 1:length(LEV)){
    ...
    mydf <- data.frame(Entrez=TT$`background_entrez[rownames(TT), "ENTREZID"]`, 
                       FC=TT$logFC,adj.P.Val = TT$adj.P.Val, PVal = TT$P.Value)
    #mydf <- mydf[mydf$PVal<0.05,]
    #mydf <- mydf$group[mydf$PVal<0.05,] <- "up"
    ...
    # mydf <- mydf[mydf$PVal<0.05,]
    # mydf$group <- "up"
    #mydf$group[mydf$PVal<0.05] <- "up"
    hh<-dim(mydf)
    if(length(hh)>0){
      for (o in seq(1,hh,by=1)){
        if(mydf[o,]$PVal<0.05){
          mydf[o,]$group="up"
        }
      }
